...
- [x] **Playing-video flicker on zoom — FIXED 2026-06-28.**

  **FINAL SHIPPED DESIGN** (the dated bullets after this are the iteration history — several approaches below, incl. the *MMR snapshot* path, were tried and **removed**; read this summary for what's actually in the code):
  1. **Surface rebind guard** — `VideoPlayerSurface`'s `AndroidView.update` rebinds the player to the `TextureView` only when the player changes (not every recomposition).
  2. **Always-offscreen video** — `VideoSurfaceChrome` *always* offscreen-composites the content (`CompositingStrategy.Offscreen`), the same path a mask already forced. The live `TextureView` is rasterized into one buffer the camera transform applies to atomically, so video tracks its Compose-drawn decoration frame with no desync/blink and no off→on toggle.
  3. **Authoritative gesture freeze** — `VideoPlaybackController.setFrozen(Boolean)` pauses every assigned player and gates `reconcile` (`shouldVideoPlay(id, paused, frozen)`), so nothing resumes/starts mid-gesture. Driven by `CanvasScreen.isNavigating` (two-finger gesture begin/end) with a `NAV_SETTLE_MS = 250` debounce so a re-grip doesn't unfreeze.
  4. **No bitmap extraction** — the offscreen *live* frame is the pixel-exact frame; the MMR snapshot / `extractVideoFrame` / `lastRenderedPositionUs` / `VideoFrameMetadataListener` / `gestureVideoFrames` / `gestureActive` were all removed.

  *Open follow-ups:* one-finger View-mode pan isn't covered by the freeze (its detector has no end callback); always-offscreen adds a per-frame offscreen pass during playback (bounded to the Full-video set).

  ---

  *Iteration history (superseded steps kept so the dead-ends aren't re-tried):*
  - *(a) Per-recomposition surface rebind.* `VideoPlayerSurface`'s `AndroidView { update = { player.setVideoTextureView(view) } }` ran on **every recomposition**; a zoom recomposes the node each frame (stable world transform, unstable node param) → ExoPlayer re-attached its output surface to the same TextureView every frame → black blip. Fix: a remembered `bound` box so `update` rebinds **only when the player changes**.
  - *(b) Live surface re-sampled through the camera transform.* Even after (a), the live `TextureView` (a real View) is re-sampled at changing bounds each frame of a pinch/zoom — it can't ride the camera `graphicsLayer` smoothly the way Compose-drawn content (poster/photo/decoration) does, so during zoom the video desyncs from its Compose-drawn **decoration frame** (most visible on framed video — the crisp frame is a stable reference for the jiggling video). *Reverted attempt:* freeze via `TextureView.getBitmap()` made it **much worse** — `getBitmap()` on the `isOpaque=false` TextureView returns null / black, so the whole gesture showed black. `getBitmap` is the wrong source.
  - *(b) fix — pause + current-frame gesture proxy (option 1, revised), implemented 2026-06-28.* `CanvasScreen` tracks `isNavigating` (set by `infiniteCanvasGestures` `onGestureBegin`/`End` — flips at gesture **boundaries**, not per frame). On gesture start `VideoPlayerSurface` **pauses playback** (`player.playWhenReady = false`, remembering the prior value) so the position is frozen — resume is seamless (no forward jump) and the proxy matches what was on screen — and draws a **stable Compose bitmap of the current frame** through the same chrome instead of the live TextureView, so it scales in lockstep with the decoration frame. On gesture end it restores `playWhenReady` (a user-paused clip stays paused) and clears the proxy → live resumes. Frame source = `extractVideoFrame(context, uri, player.currentPosition)` via `MediaMetadataRetriever.getFrameAtTime(OPTION_CLOSEST)`, run once at gesture start on `Dispatchers.IO` (**not** `getBitmap` — unreliable with `isOpaque=false`; **not** frame-0 poster — a clip at 5s must not jump to frame 0). Near the start (`≤ NEAR_START_THRESHOLD_MS = 250`) uses the resident frame-0 poster (instant + correct there). **Fallback = show live** while extraction is in flight or if it fails (`gestureSnapshot == null` → `drawContent()`): brief desync acceptable, wrong frame not. Works for **both playing and paused** (paused → its actual frozen-position frame). TextureView stays mounted (not drawn) → player never rebound/re-prepared, no LOD churn; single snapshot per gesture. Named `gestureSnapshot` / `extractVideoFrame` (not "poster"). Covers two-finger zoom/pan; one-finger View-mode pan not covered yet. *Edge:* a `reconcile` during a LOD-crossing zoom can momentarily re-set `playWhenReady` (the proxy still shows, so no visible glitch); non-LOD-crossing zoom (the main case) holds the pause.
  - *(b) unfreeze debounce, 2026-06-28.* The two-finger detector fires `onGestureBegin`/`End` each time the finger count crosses 2, so a *continuous* manipulation (re-grip, pinch↔pan) would flip `isNavigating` off→on mid-interaction → re-resume playback + re-extract the snapshot (a visible glitch). Now turning OFF is **debounced** by `NAV_SETTLE_MS = 250` in `CanvasScreen` (a `navEndJob` cancelled by the next `onGestureBegin`): the video stays paused + frozen on the one captured frame for the whole interaction, and only resumes + restores live once the fingers truly settle.
  - *(b) ownership moved to `CanvasScreen`, 2026-06-28 (fixes "frame changes mid-zoom").* Symptom: during a slow zoom the video frame kept changing. Cause: the snapshot + pause lived in `VideoPlayerSurface`, which **remounts** when a node crosses the `Full↔Simplified` LOD boundary mid-zoom → its `LaunchedEffect` re-ran → re-extracted at the (possibly pool-reset) position each remount. Fix: a single `LaunchedEffect(isNavigating)` in `CanvasScreen` owns the freeze — on gesture start it pauses every assigned player and extracts each one's current frame **once** into a `mutableStateMapOf<nodeId, ImageBitmap>` (`gestureVideoFrames`, keyed by node, surviving remounts); on settle it resumes (`gestureVideoWasPlaying`) + clears. `VideoPlayerSurface` is now dumb: it draws the passed `gestureFrame` if non-null, else live. So the frozen frame is captured once per gesture and cannot change across remounts.
  - *(b) proxy = the frame actually on screen, 2026-06-28 (fixes "proxy is a slightly-previous frame").* Extracting at `player.currentPosition` gave the wrong frame: `currentPosition` is the **ms-rounded playback clock**, not the rendered frame's PTS, and `MMR.getFrameAtTime(clock, OPTION_CLOSEST)` (separate decoder) snapped to a *neighbouring* frame (and on API < 28, `OPTION_CLOSEST` degrades toward a keyframe = clearly older). Root cause = player clock vs rendered-frame timing + MMR frame selection, **not** pause/order (a paused clip's `currentPosition` is stable). Fix: `VideoPlaybackController` now attaches a per-player `VideoFrameMetadataListener` that records each rendered frame's exact `presentationTimeUs` into a `ConcurrentHashMap` (`lastRenderedPositionUs(nodeId)`); the gesture capture extracts at *that* µs timestamp (falling back to `currentPosition*1000` only if unavailable). `extractVideoFrame` now takes µs directly (no ms→µs rounding). For a paused clip this is exactly the displayed frame → no backward jump on freeze.
  - *(b) layer instead of swap, 2026-06-28 (fixes the gesture-start blink).* `VideoPlayerSurface.drawBase` previously *swapped* live→snapshot, leaving a one-frame gap at gesture start (live hidden before the bitmap landed → a blink). Now it **always** `drawContent()` (the live, now-paused, frozen frame) and **overlays** the opaque snapshot on top once ready — so the paused live frame fills the instant before/as the snapshot arrives; never an empty/black intermediate frame.
  - *(b) decorated video blinks but masked doesn't — FIXED 2026-06-28 (offscreen parity).* On-device: masked video froze cleanly during a gesture but a *decoration-framed* video still blinked + showed a paused-frame shift. Structural cause found in `VideoSurfaceChrome`: the content `Box` got `CompositingStrategy.Offscreen` **only when `contentMask != null`**. A mask therefore composites the live `TextureView` + base draw into one **atomic offscreen buffer that tracks the Compose transform** (no View-vs-Compose desync, no inter-layer gap); a decorated-but-unmasked video took the bare path — the `TextureView` composited separately by the view system → blink + desync vs. its Compose-drawn frame. Fix: `VideoSurfaceChrome` now also offscreen-composites when a **camera gesture is active** (`gestureActive` plumbed `CanvasScreen.isNavigating` → `VideoPlayerSurface` → chrome) — the video is paused then, so the per-frame offscreen cost is nil, and a decorated video gets the same atomic path as a masked one. Confirms the hypothesis: mask "worked" because it forced the offscreen branch.
  - *(b) MMR snapshot REMOVED — replaced by offscreen-only freeze, 2026-06-28.* Investigation of the residual "paused frame shifts / changes — very strange": (1) **Not LOD** — media LOD is only `Full`-or-`Hidden` (`DEFAULT_MEDIA_MIN/MAX_RELATIVE_ZOOM = 0.01..100`, screen-cull < 2px), so a normal zoom keeps a video `Full` — no Simplified, no remount, no player churn. (The only LOD case that changes a paused frame is zooming ~100× past the Hidden threshold → evict + re-prepare to frame 0; not a normal gesture.) (2) The shift was the **MMR snapshot itself**: `getFrameAtTime` is only ~±1 frame accurate (separate decoder; `OPTION_CLOSEST` API 28+) and was drawn on top of the live frame. **Once the offscreen-during-gesture fix makes the live PAUSED frame composite atomically and track the camera (the exact frame — same path masked video uses), the snapshot is redundant and was the sole source of the shift.** So: dropped the whole snapshot path — `extractVideoFrame`, `lastRenderedPositionUs` + the `VideoFrameMetadataListener`, `gestureVideoFrames`, the off-thread capture. Gesture handling is now just: **pause all assigned videos + offscreen-composite the content (`gestureActive`); draw the live paused frame; resume on settle.** No bitmap extraction anywhere. Simpler and pixel-exact (it's the real rendered frame, not a re-decode).
  - *(b) authoritative freeze, 2026-06-28 (fixes "video timing shifts during camera transform").* The gesture pause originally only froze the videos assigned *at gesture start* (a `CanvasScreen` loop over `assignments`). But the player pool's `reconcile` sets `playWhenReady = id !in pausedNodeIds` whenever it (re)acquires a player — so a slight pan, or an evict→reacquire mid-gesture, would **resume the video under the offscreen freeze** → it advanced → timing shift. Fix: `VideoPlaybackController.setFrozen(Boolean)` — freeze pauses every assigned player and `reconcile`'s acquire now gates on `&& !frozen`, so nothing starts/resumes while a gesture is active; unfreeze restores each player to its normal state (playing unless user-paused). `CanvasScreen` calls `playbackController.setFrozen(isNavigating)`. The freeze is now authoritative across the whole pool, not a one-shot snapshot of the initial set.
  - *(b) ALWAYS-offscreen video, 2026-06-28 (fixes "frame still changes on paused video when changing camera").* Gating the offscreen layer on `gestureActive` meant it **toggled off→on at gesture start** — and toggling `compositingStrategy` on a container holding an `AndroidView`/`TextureView` re-rasterizes and shifted the frame (even on a paused clip). Masked video never had this because a mask is offscreen *always*. Fix: `VideoSurfaceChrome` now **always** offscreen-composites the content (removed the `gestureActive` gate + param all the way up). So all video uses the same atomic, transform-tracking path masked video always used — no toggle, no desync, no blink — and the gesture `setFrozen` just stops advancement. Cost: a per-frame offscreen pass while a video plays (bounded to the small Full-video set; same cost masked video already paid). `gestureActive` plumbing removed from `VideoPlayerSurface` + `CanvasScreen`.
  - *Known trade-offs:* (1) the IO frame extract has latency (~tens–hundreds ms); during it the live video shows (brief desync) — for a very quick flick the freeze may not engage at all. A per-playing-video periodic pre-snapshot (cache by uri+position bucket) would make it instant but costs continuous decode — deferred. (2) `OPTION_CLOSEST` is accurate but slower than `OPTION_CLOSEST_SYNC` (nearest keyframe); chosen for paused accuracy. (3) extracted frame is full video resolution (one bitmap per gesturing video, freed on gesture end).
- [ ] **Buffering gap:** poster is hidden the moment a clip is assigned a player, so there's a brief transparent gap before the first decoded frame. **Attempt 2026-06-28 (reverted):** painting the resident poster behind the live TextureView until `Player.Listener.onRenderedFirstFrame` made flicker *worse* on-device — likely a frame-0 poster ↔ live-position content jump each time the surface re-attaches during zoom. The static-poster gap fill is the wrong lever; revisit with a different approach (e.g. keep the *last live frame*, or suppress surface remounts across LOD) only if a gap is still visible after the rebind fix above.
...
- **Video poster (added 2026-06-28, after on-device feedback):** folded in as `AppearanceAssetKind.VideoPoster`, keyed by the video URI. `loadAppearanceBitmap` gained an optional `videoFrameMillis` (the only thing the poster path differs by); `appearanceAssetKeys` emits a poster key for video nodes so it's prewarmed + retained. `rememberVideoPosterBitmap` now reads the cache → the idle poster survives LOD remounts instead of re-extracting (the live `VideoSurfaceChrome` decoration/mask/overlay layers were already cached via the repointed loaders). Fixes the "lots of flicker on video" report. The poster's own two-layer chrome (`§ 27.6`) is unchanged.
- **Still out of scope here:** sticker *content* (renders via the content painter, not this cache; a sticker used as an appearance layer would add a `kind` later, `to_discuss.md § 30`).

Checklist — IMPLEMENTED 2026-06-28, `compileDebugKotlin` green (on-device flicker check still pending):
- [x] `AppearanceAssetKey` + `AppearanceAssetKind` + `AppearanceAssetCache` + `LocalAppearanceAssetCache` + `rememberAppearanceAssetCache()` + `appearanceAssetKeys(node)` in `feature/canvas/view/AppearanceAssetCache.kt`; cache `remember`-ed at `CanvasScaffold` and provided via `CompositionLocalProvider`.
...
- [x] **(1) Wire `colorAdjustments` into the renderer — DONE 2026-06-30.** `MediaColorFilter.kt`: `toContentColorFilter()` builds a 4×5 `ColorMatrix` `ColorFilter` for **brightness / contrast / saturation / temperature / tint / exposure**, passed to `drawCroppedBitmap`'s painter draw in `FullMediaRenderer`; **vignette** drawn separately as a radial gradient (`drawVignette`) over the content rect inside the clip. Editor lost the "not applied" notice and gained the **Tint** slider. The in-editor preview reflects it (same renderer). **Still deferred (per-pixel work):** `highlights`, `shadows`, `blur`, `sharpen` — not rendered, not exposed; the editor says so. **Video parity follow-up:** `VideoSurfaceChrome` (video poster/playback) does **not** apply the colour filter yet — images only for now.
